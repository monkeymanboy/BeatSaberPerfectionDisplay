	...
	class PerfectDisplay : MonoBehaviour
	{
		...
		private void LoadHitScore()
		{
			...
			for (var i = 0; i < judgments.Length; i++)
			{
				...
				colors[i] = "#" + ((int)(judgments[i].color[0] * 255)).ToString("X2") + ((int)(judgments[i].color[1] * 255)).ToString("X2") + ((int)(judgments[i].color[2] * 255)).ToString("X2") +
		...
		void Awake()
		{
			...
			if (shouldHitscore && IPA.Loader.PluginManager.Plugins.Any(x => x.Name == "HitScoreVisualizer"))
			{
				LoadHitScore();
			}
			else
			{
				shouldHitscore = false;
			}
		...
		private void Init()
		{
			...
			if (shouldHitscore)
			{
				sections[scoreRanges.Length].title = hitScoreNames[hitScoreNames.Length - 1];
			}
			...
			for (var i = 0; i < scoreRanges.Length; i++)
			{
				...
				if (shouldHitscore)
				{
					sections[i].title = hitScoreNames[i];
				}
			}
		...
		public void UpdateText()
		{
			if (showNumbers || showPercent)
			{
				...
				var curX = sections[0].GetWidth() / 2;
				;
				for (var i = 0; i < scoreCount.Length; i++)
				{
